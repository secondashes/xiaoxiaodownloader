...
def _pixiv_app_api(method: str, path: str, params: dict | None = None,
    ...
    App API 的 POST 参数走 query string（与 GET 相同），返回 JSON。
    """
    ...
    try:
        ...
        if search_type == "illust":
            ...
            if search_type == "novel":
                ...
                items = [c for c in (_pixiv_app_card(
                    {**(d.get("user") or {}), "is_followed": (d.get("user") or {}).get("is_followed", False)},
                    "user") for d in raw) if c["user_id"]]
        else:
            # 未登录：小说/用户走 Web ajax
            encoded = urllib.parse.quote(query, safe="")
            if search_type == "novel":
...
def _pixiv_app_items(result: dict, list_key: str, kind: str) -> list[dict]:
    ...
    for d in raw:
        if kind == "user":
            u = d.get("user") or {}
            items.append(_pixiv_app_card(u, "user"))
        else:
...
async def pixiv_feed(kind: str, page: int = 1) -> None:
    ...
    try:
        ...
        if not path.endswith("recommended") and not path.endswith("ranking"):
...
async def pixiv_follow_feed(content: str = "illust", refresh: bool = True) -> None:
    ...
    每次取最新一页（30 条）与缓存按作品 id 去重合并（新的在前，上限 600），
    ...
    try:
        ...
        seen = {c.get(id_key) for c in items}
        merged = items + [c for c in cached if c.get(id_key) not in seen][:600]
        try:
        ...
        emit({
            ...
            "search_type": kind_card, "new_count": len(items),
        })
        if merged:
            asyncio.create_task(_cache_thumbnails(merged))
        logging.info("Pixiv 关注更新 %s: 新 %d 条 / 合并后 %d 条",
                     content, len(items), len(merged))
    except PermissionError as exc:
        emit({"event": "search_error", "message": str(exc)})
    except Exception as exc:
...
async def pixiv_bookmarks(content: str = "illust", restrict: str = "public",
    ...
    try:
        ...
        result = await asyncio.to_thread(_pixiv_app_api, "GET", path, {
            "user_id": uid, "restrict": restrict, "offset": offset,
        })
        list_key = "novels" if content == "novel" else "illusts"
        items = _pixiv_app_items(result, list_key, "novel" if content == "novel" else "illust")
        if not allow_r18:
...
async def pixiv_user_page(uid: str) -> None:
    ...
    try:
        ...
        if all_items:
            ...
            asyncio.create_task(_cache_thumbnails(all_items))
        emit({
            "event": "pixiv_user_result", "user_id": uid,
            "user": {
            ...
            "profile": {
                "total_illusts": profile.get("total_illusts") or 0,
                "total_manga": profile.get("total_manga") or 0,
                "total_novels": profile.get("total_novels") or 0,
...
async def pixiv_detail(kind: str, item_id: str) -> None:
    ...
    try:
        ...
        if kind == "novel":
            detail = (await asyncio.to_thread(
                _pixiv_app_api, "GET", "/v1/novel/detail", {"novel_id": item_id})).get("novel") or {}
            # 小说正文
            text_body = await asyncio.to_thread(
                _pixiv_app_api, "GET", "/v1/novel/text", {"novel_id": item_id})
            detail["novel_text"] = text_body.get("novel_text") or ""
...
async def pixiv_novel_items(novel_id: str, title: str = "", author: str = "") -> None:
    """单个小说 → 下载条目（txt 全文 + 封面），走现有文件列表/下载管线。"""
    try:
        detail = (await asyncio.to_thread(
            _pixiv_app_api, "GET", "/v1/novel/detail", {"novel_id": novel_id})).get("novel") or {}
        title = title or detail.get("title") or f"pixiv小说_{novel_id}"
        author = author or (detail.get("user") or {}).get("name") or ""
        ...
        items = [{
            "filename": f"{sanitize_directory_name(title)}.txt",
            "size": None,
...
async def pixiv_inspect(url: str, options: dict) -> None:
    ...
    # 小说页：/novel/show.php?id={id} → txt 全文 + 封面
    m_novel = re.search(r"pixiv\.net/(?:en/)?novel/show\.php\?.*id=(\d+)", url, re.I)
    if m_novel:
        await pixiv_novel_items(m_novel.group(1))
        return
...
class DownloadManager:
    ...
    async def _pixiv_download_one(
        ...
        # 小说条目：取 /v1/novel/text 全文保存 txt（无图片直链）
        novel_id = str(item.get("novel_id") or "")
        if item.get("kind") == "novel" and novel_id:
            item["status"] = "downloading"
            self._save()
            live_manager.update_log(event="下载中", details=f"{title}（小说 txt）")
            try:
                text_body = await asyncio.to_thread(
                    _pixiv_app_api, "GET", "/v1/novel/text", {"novel_id": novel_id})
                novel_text = text_body.get("novel_text") or ""
                if not novel_text:
                ...
                fpath = _resolve_duplicate(str(Path(file_dir) / fname))
                tmp = fpath + ".part"
                ...
                self._save()
                self.emit_snapshot()
                live_manager.update_log(event="完成", details=f"{title}（txt 已保存）")
    ...
    while True:
        try:
            ...
            # ---------- Hanime1 站点 ----------
                ...
                await pixiv_follow_feed(command.get("content", "illust"))
